...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(createMenu().order('coxinha')).toEqual(expect.arrayContaining(objetoRetornado.consumption));

    expect(createMenu().order('chocolate') && createMenu().order('bala') && createMenu().order('guaraná')).toEqual(expect.arrayContaining(objetoRetornado.consumption));

    expect(createMenu().order('chocolate') && createMenu().order('chocolate') && createMenu().order('chocolate')).toEqual(expect.arrayContaining(objetoRetornado.consumption));

    let teste = createMenu({ food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} });
    teste.order('coxinha');
    teste.order('cerveja');
    expect(teste.pay()).toBeCloseTo(11.88, 1);
